...
thisExp = data.ExperimentHandler(name=expName, version='',
                                 ...
                                 originPath='C:\\Users\\adm\\Documents\\Projects\\Npx-Experiment\\PsychopyTasks\\Psyexps\\VisualReceptiveField.py',
...
for thisTrial in trials:
    ...
    # --- Run Routine "trial" ---
    while continueRoutine and routineTimer.getTime() < 0.32:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        ...
        if photo_diod_rect.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > photo_diod_rect.tStartRefresh + 0.020 - frameTolerance:
                # keep track of stop time/frame for later
        ...
        if black_screen.status == NOT_STARTED and tThisFlip >= 0.020 - frameTolerance:
            # keep track of start time/frame for later
        ...
        if stimulus.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > stimulus.tStartRefresh + 0.020 - frameTolerance:
                # keep track of stop time/frame for later
