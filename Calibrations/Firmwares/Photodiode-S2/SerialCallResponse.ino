...
*/
int photodiod_voltage = 0;
int digital_potentiometer = 0;
int analog_potentiometer = 0;    // first analog sensor
int switch_potentiometer = 0;
int status_flag = 0;

String inSringCommand;         // incoming serial byte
int time_keeper = 0;
...
void setup() {
  ...
  pinMode(comp_out,INPUT);
  pinMode(comp_out+1,INPUT);
...
void loop() {
  ...
  if (Serial.available() > 0) {
    // get incoming byte:
    inSringCommand = Serial.readString();
    int command = inSringCommand.toInt();
    if(command == 0){
       photodiod_voltage = analogRead(A5);
       Serial.println(photodiod_voltage);
    }
    else if(command == 10){
       digital_potentiometer = analogRead(A1);
       Serial.println(digital_potentiometer);
    }
    else if(command == 11){ // increment
      Serial.println(1);
      time_keeper = 1;
    }
    else if(command == 12){ // decrement
      Serial.println(1);
      time_keeper = -1;
    }
    else if(command == 13){
    ...
    // Group 2X
      ...
      analog_potentiometer = analogRead(A2);
      Serial.println(analog_potentiometer);
    }
    else if(command == 30){
       switch_potentiometer = analogRead(A3);
      Serial.println(switch_potentiometer);
    }
    else if(command == 100){
      status_flag = 1;
  ...
  // Update Digital Potentiometer if its required
  if(time_keeper>0){
    potUp();
    time_keeper--;
  }
  ...
  //Update Status Flag
  digitalWrite(status_led, status_flag);
}
